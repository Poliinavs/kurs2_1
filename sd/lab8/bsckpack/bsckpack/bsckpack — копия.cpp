...
void BackPack(const vector<int>& weight, const vector<int>& cost, const vector<string> name, int size)
{
    ...
    for (size_t i = 1; i <= n; i++)
    {
        ...
        {
            if (weight[i - 1] <= j)//след элемент больше предыдущего
            {
                if (dp[j][i - 1] > dp[j - weight[i - 1]][i - 1] + cost[i - 1]) //если вес ,,,
                {
                    dp[j][i] = dp[j][i - 1];
                }
                else
                {
                    dp[j][i] = dp[j - weight[i - 1]][i - 1] + cost[i - 1]; //иначе перезаписываем
                }
            }
            else
            {
                dp[j][i] = dp[j][i - 1]; //предыдущий элемент больше текущего следоват продлеваем его значение на текущий 
            }
        }
    }
    for (int i = 0; i <= n; i++)
    ...
    while (true)
    {
        ...
        if (dp[j][i] == dp[j][i - 1]) //перед текущим элементом стоит еще один такой же следоват его брать не надо
        {
            need[i - 1] = false;
            i--;
        }
        ...
        {
            j = size - weight[i - 1]; //от размера табл отнимаем вес данной ячейки=>перемещ на нужн столбец
            i--;
        }
    }
    for (int i = 0; i < n; i++)
