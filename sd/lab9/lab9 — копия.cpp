...
string mutatedGene(string gnome) //случайн образ выбираем две вершиныих меняем местами
{
	while (true) {
		int r = rand_num(1, V);
		int r1 = rand_num(1, V);
...
string create_gnome() //создаем начальную последоват без повтрений номеров генов с 0 первым 02 04 взвращем string
{
	string gnome = "0";
	while (true) {
		if (gnome.size() == V) {
...
void TSPUtil()
{
	...
	for (int i = 0; i < populationSize; i++) { //колич строк
		temp.gnome = create_gnome();   //создаем уникальный геном
		temp.weight = cal_weight(temp.gnome); //находим весь данного генома
		population.push_back(temp); //закидываем строку
	}
	...
	int mutation_proc = 10000; //заданный процент мутации ббольше 100

	sort(population.begin(), population.end(), lessthan); //сортруем по весу о наим к наиб

	while (mutation_proc > 100 && numb_gen <= evoCount) {
		cout << endl << "Лучший геном: " << population[0].gnome;
		cout << " его вес: " << population[0].weight << "\n\n";

		/*cout << "\nтекущий буфер: " << mutation_proc << "\n";*/
		vector<struct individual> new_population;

 		for (int i = 0; i < child; i++) {//берем первые строки по количеству детей и производ мутацию, если вес примутации меньше родителя отсавл, в противн случ вес будет максим 
		...
		{
			population.push_back(new_population[i]);//new popul это с мутацией
		}
		sort(population.begin(), population.end(), lessthan);

		for (int i = 0; i < child; i++) //удалаяем последние как наихудшие
		{
			population.pop_back();
		}
